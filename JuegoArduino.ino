...
#define   ENTRADA_C  A2
#define   ENTRADA_D  A3
#define   ENTRADA_VALIDA  A5
#define   SALIDA_A   2
#define   SALIDA_B   3
#define   SALIDA_C   4
#define   SALIDA_D   5
#define   SALIDA_ERROR   6
...
void leeSecuencia(){
	...
	flag = 0;
	while(flag == 0){
		if(digitalRead(ENTRADA_VALIDA) == LOW){				
			for(int x = 0; x < nivelActual; x++){
				if(secuenciaUsuario[x] != secuencia[x]){
					secuenciaError();
					return;
				}
			}
